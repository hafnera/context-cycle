...
7. **Full extracts by default:** neither the skill nor the hooks truncate on their own (`--last`/`--max-chars` only on explicit user instruction; full-extract rule in SKILL.md). Every import reports its size: `Imported context: ~Xk tokens ≈ Y% of the …-token context window`.
