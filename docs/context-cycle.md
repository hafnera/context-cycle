...
7. **Detail level:** the skill asks the user (question card) which parts to import — Full / without subagent full reports / final answers only / minimal; the post-compact hook always injects Full. **Full extracts by default:** neither the skill nor the hooks truncate on their own (`--last`/`--max-chars` only on explicit user instruction; full-extract rule in SKILL.md). Every import reports its size: `Imported context: ~Xk tokens ≈ Y% of the …-token context window`.
